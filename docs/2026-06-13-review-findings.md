...
**위치**: `RTWWebServer/Services/UserService.cs:90`,
`RTWWebServer/Services/GameEntryService.cs:66`
...
### 적용된 수정
`UpdateNicknameAsync`의 `ValidateNickname`이 `^User_\d+$` 패턴을 예약어로 거부한다
(`UserService.cs:90`, `ReservedNicknamePattern`). 이것만으로 `User_{accountId}`의 전역
유일성이 구조적으로 보장된다: accountId가 유일하므로 기본 닉네임끼리 충돌하지 않고, 타 계정은
예약 검증 때문에 `User_\d+` 형태를 가질 수 없다.

따라서 유저 생성 측에는 대체 닉네임 재시도를 **두지 않는다** — 예약 검증 하에서는 닉네임 충돌이
불가능해 재시도 분기가 도달 불가능한 dead code가 되기 때문이다. `GameEntryService`의 catch는
`uk_account_id` 경합(락 TTL 만료·failover로 동일 account가 동시 생성되는 경우)만 단일 재조회로
처리한다(`GameEntryService.cs:66`).

> **레거시 데이터 미고려**: 예약 검증 도입 이전(`acb2b8b`~)에 누군가 `User_\d+` 닉네임을
> 이미 설정했다면, 해당 account의 최초 입장은 여전히 실패한다. 그린필드 가정으로 단순화했으며,
> 필요 시 기존 `User_\d+` 닉네임을 정리하는 데이터 마이그레이션으로 대응한다.
...
- **게임 서버 인증 스텁**: 비어 있지 않은 토큰을 모두 통과. 웹서버 세션(`session_{userId}`)과
  대조하려면 userId가 필요하다. proto 원본은 리포에 있고(`NetworkDefinition/Proto/Source/packet.proto`)
  빌드 시 자동 생성되므로 "proto 부재"가 블로커는 아니다. 실제 블로커는 (1) `CAuthToken`에 userId 필드
  없음, (2) 게임서버에 Redis 접근 수단 자체가 없음 — 상세·해소 방안은
  `docs/2026-06-13-remaining-work-sequencing.md` 묶음4 참고.
