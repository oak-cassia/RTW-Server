...
| 순서 | 묶음 | 포함 | 트랙 | 변경 범위 | 새 인프라 결정 | 상태 |
|---|---|---|---|---|---|---|
| 0 | 기록 정정 | proto 블로커 문구 수정 | 문서 | 사소 | 없음 | ✅ 완료(2026-06-13) |
| 1 | 웹서버 트랜잭션·닉네임 | P3(=M1) + M5 | 정합성 | 작음 | 없음 | ✅ 완료 |
| 2 | 캐시 정합성 | P2(=M4) + P4(통합테스트) | 정합성 | 중간 | 테스트 인프라 | ✅ 완료(2026-06-13, SQLite) |
| 3 | 게임서버 신뢰경계 | M3 + L2 | 게임서버 | 작음 | 없음 | ⏳ 다음 |
...
## 묶음 2 — 캐시 정합성 (P2 + P4) ✅ 완료(2026-06-13)

**구현 결과**: `CacheManager`/`RequestScopedLocalCache` 계열 4파일 + 그 테스트 제거(→ L3의
`RollbackAllChanges` 제거 흡수). `IDistributedCacheAdapter` 위에 얇은
[IPlayerCharacterCache](../RTWWebServer/Cache/PlayerCharacterCache.cs)(cache-aside, **60초 TTL**, DI Singleton)로
교체. `CharacterGachaService`는 새 캐시로 전환(키 생성·dirty-key·commit/rollback 머신 제거). P4 통합테스트는
**관계형 SQLite(in-memory)** 채택([CharacterGachaServiceIntegrationTests.cs](../RTWTest/WebServer/Services/CharacterGachaServiceIntegrationTests.cs)).
테스트 프로젝트 EF 패키지를 **8.0.13으로 정렬**(웹서버 Pomelo 8.0.3과 일치) — EF9를 끌어오면
`ExecuteUpdateAsync` 시그니처 불일치로 `MissingMethodException` 발생하기 때문. 빌드 0경고·**61테스트 통과**.
...
- **정합성 트랙(완료)**: P1✓ → 묶음1(P3·M5)✓ → 묶음2(P2·P4)✓
- **게임서버 트랙(다음)**: 묶음3(M3·L2) → 묶음4(인증 완성)
